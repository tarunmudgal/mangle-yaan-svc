...
from lib.common.fault_injector import *
...
def get_env_var():
    ...
    env_dict["fault_percentage"] = os.getenv("fault_percentage", 50)
    env_dict["timeout"] = os.getenv("timeout", 10)
...
if __name__ == "__main__":
    """
    Usage:
    python test_runner.py -p project_name -w workload_name -r run_id
    """

    project_name, workload_name, run_id = check_arg(sys.argv[1:])

    env_dict = get_env_var()
    mylog.info("*** Env variable dictionary %s ***", env_dict)
    ...
    if "cpuload" in env_dict["fault_type"]:
        ...
        mylog.info("*** env params %s ***", env_dict)
    elif "memload" in env_dict["fault_type"]:
        env_dict["fault_id"] = inject_infra_memory_fault(env_dict)
        mylog.info("*** env params %s ***", env_dict)
    env_dict["fault_start_date"] = str(fault_start_date)
    env_dict["fault_end_timestamp"] = str(
        (fault_start_date + datetime.timedelta(0, int(env_dict["timeout"]) / 1000)).timestamp()
    )

    mylog.info(
        "*** Fault %s is injected with Fault Id %s ***",
        env_dict["fault_type"],
        env_dict["fault_id"],
    )
    write_input_file(project_name, workload_name, env_dict, "fault.csv")
