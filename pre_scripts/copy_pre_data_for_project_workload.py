...
class Copy_Inputs_For_Project_Workload(object):
    ...
    def upload_inputs_to_s3(self):
        ...
        directory_name = 'maxim-gun/' + self.project_name + "/" + self.workload_name + "/" + self.run_id + "/inputs"
        ...
        try:
            for path, subdirs, files in os.walk(input_dir):
                for file in files:
                    ...
                    s3_file_location = os.path.normpath(directory_name + '/' + dest_path + '/' + file)
                    local_file_location = os.path.join(path, file)
                    print("Uploading: [", local_file_location, "] to S3 location: [",
                          s3_file_location + "]")
