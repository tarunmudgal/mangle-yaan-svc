...
from mangle_lib.mangle_api import MangleApi
from mangle_lib.operations.common_operations import CommonOps
from mangle_lib.operations.endpoint_operations import EndpointOperations
...
class FaultBase(object):
    ...
    def mangleapi(self):
        ...
        # TODO:
        """
        return MangleApi(self.mangle_ip, self.mangle_username,
    ...
    def inject_daemon_fault(self, fault_type, **kwargs):
        ...
        # TODO:
        """
        fault_thread = threading.Thread(target=self.inject_fault,
                                        args=fault_type, kwargs=kwargs)
    ...
    def invoke_mangle_api(self, verb, api_endpoint, payload, fault_area,
                          fault_type):
        """
        # TODO:
        """
    ...
    def get_process_id(self, svc_name):
        ...
        process_identifier = {
            "corfu": "truststore.password"

        }

        # build command
        cmd = "ps -ef|awk '$1 == \"%s\"'|grep %s|awk '{print $2}'" % \
              (svc_name, process_identifier[svc_name])
        log.debug("%s *** running command: %s on node %s ***",
                  logger.plugin_name, cmd, self.nsxt_ip)
        pid = self.nsxt_obj.get_cli_output(cmd)
        log.debug("%s *** pid is %s ***", logger.plugin_name, pid)
        return pid
    ...
    def set_schedule(schedule_epoch_time, schedule_cron_exp):
        ...
        if schedule_cron_exp is not None and schedule_epoch_time is not None:
            raise Exception("%s *** either provide schedule_cron_exp"
                            " or schedule_epoch_time", logger.plugin_name)
    ...
    def _log_fault(fault_area, fault_type, payload, success_msg=0):
        ...
        if success_msg:
            log.debug("%s *** %s::%s FAULT INJECTED SUCCESSFULLY ***",
                      logger.plugin_name, fault_area, fault_type)
            log.banner("%s *** %s::%s FAULT INJECTED SUCCESSFULLY ***",
                       logger.plugin_name, fault_area, fault_type)
        else:
            log.banner("%s *** GENERATING %s::%s ***", logger.plugin_name,
                       fault_area, fault_type)
            log.debug("%s *** PAYLOAD FOR %s::%s *** \n %s", logger.plugin_name,
...
class InfraFaultOperations(FaultBase):
    ...
    def generate_memory_fault(
        ...
        self.invoke_mangle_api(VERB_POST, self.memory_endpoint, payload,
                               fault_area, fault_type)
    ...
    def generate_diskio_fault(self, iosize, target_dir, timeout,
        ...
        self.invoke_mangle_api(VERB_POST, self.diskio_endpoint, payload,
                               fault_area, fault_type)
    ...
    def generate_processkill_fault(self, process_name=None, process_id=None,
        ...
        self.invoke_mangle_api(VERB_POST, self.kill_process_endpoint, payload,
                               fault_area, fault_type)
    ...
    def generate_file_handler_leak_fault(self, timeout,
        ...
        self.invoke_mangle_api(VERB_POST, self.filehandler_leak_endpoint,
                               payload, fault_area, fault_type)

    def generate_kernel_panic_fault(self, injection_homedir="/tmp",
                                    schedule_epoch_time=None,
                                    schedule_cron_exp=None, tags={}):
        """
        # TODO:
        ...
        self.invoke_mangle_api(VERB_POST, self.kernel_panic_endpoint, payload,
                               fault_area, fault_type)

    def generate_diskspace_fault(self, diskload, target_dir, timeout, injection_homedir="/tmp",
                              schedule_epoch_time=None, schedule_cron_exp=None, tags={}):
        """
        # TODO:
        """
        ...
        self.invoke_mangle_api(VERB_POST, self.diskspace_endpoint, payload,
                               fault_area, fault_type)
    ...
    def generate_network_fault(self, nw_fault_type, nicname, timeout, injection_homedir="/tmp",
        ...
        self.invoke_mangle_api(VERB_POST, self.network_endpoint, payload,
                               fault_area, fault_type)
...
class AppFaultOperations(FaultBase):
    ...
    def generate_cpu_fault(self, cpuload, timeout, java_home_path, jvm_process,
                           user, port=9091, injection_homedir="/tmp",
                           schedule_epoch_time=None, schedule_cron_exp=None,
                           tags={}):
        """
        # TODO:
        ...
        payload = {
            ...
            "jvmProperties": {"javaHomePath": java_home_path,
                              "jvmprocess": int(jvm_process), "user": user,
                              "port": port}
        }
    ...
    def generate_memory_fault(
            ...
            port=9091, injection_homedir="/tmp", schedule_epoch_time=None,
            schedule_cron_exp=None, tags={}):
        """
        # TODO:
        """
    ...
    def generate_thread_leak(self, timeout, java_home_path, jvm_process, user,
                             out_of_memory_req=False, port=9091,
        ...
        if isinstance(jvm_process, str):
            log.error("%s *** 'jvm_process' pass pid instead of porcess name",
                      logger.plugin_name)
            raise Exception()
        ...
        self.invoke_mangle_api(VERB_POST, self.thread_leak_endpoint, payload,
                               fault_area, fault_type)