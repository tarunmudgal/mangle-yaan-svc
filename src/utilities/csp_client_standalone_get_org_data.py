...
if __name__ == "__main__":
    ...
    ORGS_FILE_PATH = "preview_orgs_list.txt"
    ...
    # ORGS_USERS = "/am/api/v2/orgs/{orgId}/users"
    ORGS_USERS = "/am/api/v2/orgs/{orgId}/users"
    ORGS_ROLES = "/am/api/orgs/{orgId}/roles"
    ORGS_GROUPS = "/am/api/orgs/{orgId}/groups"
    ORGS_OAUTH_APPS = "/am/api/orgs/{orgId}/oauth-apps"
    ORGS_SERVICES = "/slc/api/v2/orgs/{orgId}/services"
    ...
    with open("preview_orgs_data.csv", "w") as csvfile:
        fields = ["orgs", "no_of_users", "no_of_orgs_roles", "no_of_groups", "no_of_oauthApps", "no_of_services", "no_of_serviceRoles"]
        ...
        with open(ORGS_FILE_PATH) as fp:
            for org_id in fp:
                ...
                org_users_resp = cclient.make_call(
                    "GET",
                    ORGS_USERS.format(orgId=org_id)
                )
                mylog.info("org_users_resp={}".format(org_users_resp))
                if org_users_resp.status_code != 200:
                    mylog.error(
                        "Error occurred for GET {} for org={}".format(ORGS_USERS, org_id)
                    )
                    continue

                no_of_users = org_users_resp.json.get("totalResults")

                # Get all org roles in an org
                org_roles_resp = cclient.make_call(
                    "GET",
                    ORGS_ROLES.format(orgId=org_id)
                )
                mylog.info("org_users_resp={}".format(org_roles_resp))
                if org_roles_resp.status_code != 200:
                    mylog.error(
                        "Error occurred for GET {} for org={}".format(ORGS_ROLES, org_id)
                    )
                no_of_roles = len(org_roles_resp.json.get("orgRolesData"))

                # Get all Orgs Group in an org
                org_groups_resp = cclient.make_call(
                    "GET",
                    ORGS_GROUPS.format(orgId=org_id)
                )
                mylog.info("org_users_resp={}".format(org_groups_resp))
                if org_groups_resp.status_code != 200:
                    mylog.error(
                        "Error occurred for GET {} for org={}".format(ORGS_GROUPS, org_id)
                    )
                no_of_groups = org_groups_resp.json.get("totalResults")

                # Get all Oauth Apps in an org
                org_oauthapps_resp = cclient.make_call(
                    "GET",
                    ORGS_OAUTH_APPS.format(orgId=org_id)
                )
                mylog.info("org_users_resp={}".format(org_oauthapps_resp))
                if org_oauthapps_resp.status_code != 200:
                    mylog.error(
                        "Error occurred for GET {} for org={}".format(ORGS_OAUTH_APPS, org_id)
                    )
                no_of_oauthApps = org_oauthapps_resp.json.get("totalResults")

                # Get all services and serviceRoles in an org
                org_services_resp = cclient.make_call(
                    "GET",
                    ORGS_SERVICES.format(orgId=org_id)
                )
                mylog.info("org_users_resp={}".format(org_services_resp))
                if org_services_resp.status_code != 200:
                    mylog.error(
                        "Error occurred for GET {} for org={}".format(ORGS_SERVICES, org_id)
                    )
                results_services_len = len(org_services_resp.json.get("results"))
                if results_services_len > 0:
                    no_of_services = len(org_services_resp.json.get("results")[0]["services"])
                    no_of_serviceRoles = str(org_services_resp.json).count("refLink")
                else:
                    no_of_services = 0
                    no_of_serviceRoles = 0

                csv_writer.writerow([org_id, no_of_users, no_of_roles, no_of_groups, no_of_oauthApps, no_of_services, no_of_serviceRoles])
