                ...
                for count in range(loop_count):
                    ...
                    for result in get_org_users_resp.json.get('results'):
                        for org_role in result.get('organizationRoles'):
                            domain = result.get("user").get("email").split('@')[1]
                            if org_role.get('name') == "org_owner":
                                org_owners.append(result.get("user").get("email"))
                                if domain not in total_domains:
                                    total_domains[domain] = 1
                                total_domains[domain] += 1
                    page_start += PAGE_SIZE
                    total_org_owners += len(org_owners)
                mylog.info("org_id={} total_org_owners={}".format(org_id, total_org_owners))
    mylog.info("total_domains={}".format(total_domains))
