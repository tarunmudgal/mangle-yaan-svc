...
if __name__ == "__main__":
    ...
    ORG_FILE_PATH = "trustee_orgs.txt"
    ...
    ORGS_TRUST = "/am/api/orgs/{orgId}/trusts"

    # initialize csp rest client
    cclient = CSPClient(
        "console-stg.cloud.vmware.com",
        "CcTpaO3cRA_s2p-Q4xvYBZjOT_BWKDshJqt52lA3qO_w5rbGDiGduovaay5W857R",
        timeout=120,
    )

    # use case: remove service access from given orgs
    with open("trustee_orgs.csv", "w") as csvfile:
        fields = ["trustee_org", "trust_org", "status"]
        csv_writer = csv.writer(csvfile)
        csv_writer.writerow(fields)
        with open(ORG_FILE_PATH) as fp:
            for org_id in fp:
                org_id = org_id.strip()

                mylog.info("processing service_id={}".format(org_id))

                # Get all trusts in an org
                org_trust_resp = cclient.make_call(
                    "GET",
                    ORGS_TRUST.format(orgId=org_id)
                )

                if org_trust_resp.status_code != 200:
                    mylog.error(
                        "Error occurred for GET {} for org={}".format(ORGS_TRUST, org_id)
                    )
                    continue

                count = org_trust_resp.json.get("totalResults")

                mylog.info("total occurence={}".format(count))
                ...
                for i in range(count):
                    status = org_trust_resp.json.get("results")[i]["status"]
                    trust_org = org_trust_resp.json.get("results")[i]["trustedOrg"].get("id")
                    trustee_org = org_trust_resp.json.get("results")[i]["trusteeOrg"].get("id")
                    csv_writer.writerow([trustee_org, trust_org, status])
