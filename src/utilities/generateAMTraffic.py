...
if __name__ == "__main__":
    ...
    ORG_USERS_INFO = "/am/api/users/{userID}/orgs/{orgID}/info"
    PAGE_SIZE = 100
    ...
    list_of_blocks = list(CSPClient.divide_chunks(list_of_users,config.BLOCK_SIZE))
    finalResult_avg = []
    finalResult_max = []

    #making call for each user block
    mylog.info("Test is started with {} sec sleep between each block".format(config.SLEEP_BETWEEN_BLOCKS))
    for index, block in enumerate(list_of_blocks):
        tempResult = []
        for user in block:
            username = user.split(",")[0]
            get_users_info = cclient.make_call("GET",ORG_USERS_INFO.format(userID=username, orgID=config.ORG_ID))
            tempResult.append(get_users_info.resp_time)
        max_latency=max(tempResult)
        average_latency=sum(tempResult) / len(tempResult)
        mylog.info("Result for block {} ==> Max:{} sec  Avg:{} sec".format(index+1,max_latency,round(average_latency,2)))
        finalResult_avg.append(average_latency)
        finalResult_max.append(max_latency)
        # sleeping for some configured time to get some more space to AM.
        mylog.debug("Sleeping for {} sec".format(config.SLEEP_BETWEEN_BLOCKS))
        time.sleep(config.SLEEP_BETWEEN_BLOCKS)


    mylog.info("Test is completed. Generating report...")
    average_latency_final = round(sum(finalResult_avg) / len(finalResult_avg), 2)
    max_latency_final = max(finalResult_max)
    mylog.info("Average latency of all calls --> {} sec \n Max latency of all calls --> {} sec".format(average_latency_final,max_latency_final))
