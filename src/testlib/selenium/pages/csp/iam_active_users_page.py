...
class ActiveUsersPage(BasePage):
    def __init__(self, driver, timeout=30):
        ...
        self.iam_active_users_locators = iam_active_users_page.IAMActiveUsersLocators
    ...
    def verify_page_loaded_correctly_when_commerce_down(self):
        ...
        ), "could not find active users search locator={}".format(
            self.iam_active_users_locators.NAV_ACTIVE_USERS_SEARCH
        )
        assert self.if_element_exists(
    ...
    def goto_active_users(self):
        indentity_access_management_btn = self.find_element(
            self.iam_active_users_locators.BTN_IDENTITY_AND_ACCESS_MANAGEMENT
        )
        if indentity_access_management_btn:
            indentity_access_management_btn.click()
            active_users_page_link = self.find_element(
                self.iam_active_users_locators.NAV_ACTIVE_USERS
            )
            if active_users_page_link:
                active_users_page_link.click()
                self.wait_for_element(self.iam_active_users_locators.TXT_ACTIVE_USERS)
                assert (
                    self.find_element(self.iam_active_users_locators.TXT_ACTIVE_USERS).text
                    == "Active Users"
                ), "Active users page is not working as expected"
