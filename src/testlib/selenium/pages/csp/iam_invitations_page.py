...
class InvitationsPage(BasePage):
    def __init__(self, driver, timeout=30):
        ...
        self.iam_invitations_locators = iam_invitations_page.IAMInvitationsLocators
        super().__init__(driver, self.current_url, timeout=timeout)

    def goto_invitations_page(self):
        invitation_page_link = self.find_element(
            self.iam_invitations_locators.NAV_PENDING_INVITATIONS
        )
        if invitation_page_link:
            invitation_page_link.click()
            self.wait_for_element(self.iam_invitations_locators.TXT_PENDING_INVITATIONS)
            assert (
                self.find_element(self.iam_invitations_locators.TXT_PENDING_INVITATIONS).text
                == "Pending Invitations"
            ), "Pending Invitations page is not working as expected"
            ...
            mylog.error(
                "Pending Invitation page could not be located using locator={}".format(
                    self.iam_invitations_locators.NAV_PENDING_INVITATIONS
                )
            )
