...
class OauthAppsPage(BasePage):
    def __init__(self, driver, timeout=30):
        ...
        self.iam_oauth_apps_locators = iam_oauth_apps_page.IAMOauthAppsLocators
    ...
    def goto_oauth_app_page(self):
        oauth_apps_page_link = self.find_element(self.iam_oauth_apps_locators.NAV_OAUTH_APPS)
        if oauth_apps_page_link:
            oauth_apps_page_link.click()
            self.wait_for_element(self.iam_oauth_apps_locators.TXT_OAUTH_APPS)
            assert (
                self.find_element(self.iam_oauth_apps_locators.TXT_OAUTH_APPS).text == "OAuth Apps"
