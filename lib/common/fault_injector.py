...
from lib.mangle.operations import InfraFaultOperations
...
def inject_infra_cpu_fault(request):
    mylog.info("Inject CPU Fault via Fixture..")
    ifo = InfraFaultOperations(
        MANGLE_IP,
        MANGLE_USER,
        MANGLE_PASSWORD,
        request["k8s_endpoint"],
        request["k8s_credential"],
    ...
    return ifo.inject_fault(
        "CPU",
        cpuload=int(request["fault_percentage"]),
...
def inject_infra_memory_fault(request):
    mylog.info("Inject MEMORY Fault via Fixture..")
    ifo = InfraFaultOperations(
        MANGLE_IP,
        MANGLE_USER,
        MANGLE_PASSWORD,
    ...
    return ifo.inject_fault(
        "MEMORY",
        memoryload=int(request["fault_percentage"]),
        timeout=int(request["timeout"]),
        container_name=request["container_name"],
        label=request["container_label"],
        schedule_cron_exp=schedule_cron_exp,
