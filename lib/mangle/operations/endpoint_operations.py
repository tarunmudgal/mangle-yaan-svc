...
from lib.mangle.endpoint import *
...
class EndpointOperations(object):
    ...
    def setup_fault_infra(
        self, mangle_seesion_obj, k8s_endpoint, k8s_credential, k8s_namespace, ep_name
    ):
        """
        Create a setup like create endpoint endpoint if doesn't exist
        """
        # import pdb; pdb.set_trace()
        endpoints_obj = Endpoint(mangle_seesion_obj)
        endpoint_credential_obj = EndpointCredential(mangle_seesion_obj)
        test_connection_obj = TestConnection(mangle_seesion_obj)
        mymymylog.banner("%s *** Creating Endpoint Credential ***", logger.plugin_name)
        mymylog.debug(
            "%s *** endpoint credential name is '%s' ***", logger.plugin_name, k8s_credential
        )
        # Creating new endpoint credential if does not exist
        if self.is_endpoint_credential_exist(endpoint_credential_obj, k8s_credential):
            mylog.debug(
                "%s *** Endpoint credential %s already exists. Skipping" " creating it ***",
                logger.plugin_name,
                k8s_endpoint,
            )
        else:
            kubeconfig_dir = [
                os.path.join(file + "/config/")
                for file in sys.path
                if file.endswith("mangle-yaan-service")
            ][0]
            multipart_form_data = [("kubeConfig", open(kubeconfig_dir + ep_name + ".yaml", "rb"))]
            status, output = endpoint_credential_obj.create(k8s_credential, multipart_form_data)
            print(output)
            mylog.debug(
                "%s *** output of endpoint credential create obj ***" " \n %s",
                logger.plugin_name,
                output,
            )
            ...
            payload = (
                '{"name": "%s", "endPointType": "K8S_CLUSTER",'
                '"credentialsName": "%s",'
                '"k8sConnectionProperties": '
                '{"namespace": "%s"}}' % (k8s_endpoint, k8s_credential, k8s_namespace)
            )

            print(payload)
            mylog.debug(
                "%s *** payload for endpoint & test-connection *** \n %s",
                logger.plugin_name,
                payload,
            )

            # Testing connection for endpoint credential
            mymylog.banner("%s *** Testing Endpoint Connection ***", logger.plugin_name)
            ...
            mymylog.banner("%s *** Creating Endpoint ***", logger.plugin_name)
            if self.is_endpoint_exist(endpoints_obj, k8s_endpoint):
                mylog.debug(
                    "%s *** Endpoint %s already exists. Skipping creating" " it ***",
                    logger.plugin_name,
                    k8s_endpoint,
                )
            else:
                ...
                mylog.debug(
                    "%s *** output of endpoint create obj *** \n%s", logger.plugin_name, output
                )

    @utilities.retry(retries=5, exceptions=Exception, sleep=10)
    def is_endpoint_credential_exist(self, endpoint_credential_obj, endpoint_credential_name):
        """
    ...
    @utilities.retry(retries=5, exceptions=Exception, sleep=10)
    def delete_endpoint_credential(self, endpoint_credential_obj, endpoint_credential_name):
        """
        Checks if endpoint credential exists
        # TODO : Add try except block
        """
        status, output = endpoint_credential_obj.delete(endpoint_credential_name)

        if not status:
            mylog.error(
                "%s *** DELETE endpoint credential API FAILED with" " output *** \n %s",
                logger.plugin_name,
                output,
            )

        if not self.is_endpoint_credential_exist(
            endpoint_credential_obj, endpoint_credential_name
        ):
            mylog.debug(
                "%s *** Endpoint credential %s deletion SUCCESSFUL ***",
                logger.plugin_name,
    ...
    def delete_endpoint(self, endpoints_obj, endpoint_name):
        ...
        if not status:
            mylog.error(
                "%s *** DELETE endpoint API FAILED with output ***" " \n %s",
                logger.plugin_name,
                output,
            )
        ...
        if not self.is_endpoint_exist(endpoints_obj, endpoint_name):
            mylog.debug(
                "%s *** Endpoint %s deletion SUCCESSFUL ***", logger.plugin_name, endpoint_name
            )

    def wait_for_endpoint_credential_to_create(
        self, endpoint_credential_obj, endpoint_credential_name
    ):
        """
            ...
            if status:
                for item in range(len(output)):
                    if endpoint_credential_name in output[item]["name"]:
                        mylog.debug(
                            "%s *** endpoint '%s' created successfully" " ***",
                            logger.plugin_name,
                            endpoint_credential_name,
                        )
                        found = 1
                ...
                if found == 1:
                    break
                retry += 1
                mylog.debug("%s *** Waiting for endpoint creation to finish ***", logger.plugin_name)
